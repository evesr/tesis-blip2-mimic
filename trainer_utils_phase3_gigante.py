#!/usr/bin/env python3
"""
trainer_utils_phase3_gigante.py — Utilidades de entrenamiento para Fase 3 Gigante
==================================================================================
Expansión de Rango LoRA Masiva (r=128): Optimización del Q-Former con bases LLM+ViT congeladas.
...
# Fase 3.2 trabaja con el 10% del dataset (escalamiento final)
...
# Solo Base B (best_model_Config_B1) — línea base para la expansión masiva
...
# Directorio raíz de salida para la Fase 3 Gigante (r=128)
OUTPUT_DIR      = BASE_DIR / "phase3_results-gigante"
...
def inject_qformer_lora(
    ...
    print(f"   [qformer_lora] Descongelando las últimas 2 capas del ViT...")
    ...
    n_vit_unfrozen = sum(p.numel() for n, p in base_model.vision_model.named_parameters()
                         if p.requires_grad)
    print(f"   [qformer_lora] Q-Former entrenables  : {n_qformer:,} params")
    print(f"   [qformer_lora] ViT capas 37-38        : {n_vit_unfrozen:,} params")
    ...
    print(
        f"\n   ┌─── Parámetros del modelo (Fase 3 Gigante: Q-Former LoRA r=128) ──┐\n"
        f"   │  Total        : {total:>15,}                          │\n"
...
class Phase3AuditCallback(TrainerCallback):
    ...
    def __init__(
        ...
        self.audit_path.write_text(
            f"# Audit Log — Fase 3 Gigante (r=128, 10%) — {config_name}\n"
...
def train_single_config_phase3(
    ...
    print(f"\n{'='*80}")
    print(f"  FASE 3 GIGANTE (10%) — Config {config_num}/{total_configs}: {name}")
...
def plot_comparative_curves(
    ...
    specs = [
        ("Val_Loss", "Val Loss",  "01_val_loss_phase3_gigante.png"),
        ("BLEU-4",   "BLEU-4",   "02_bleu4_phase3_gigante.png"),
        ("ROUGE-L",  "ROUGE-L",  "03_rougeL_phase3_gigante.png"),
    ]
    ...
    for col, ylabel, filename in specs:
        ...
        ax.set_title(
            f"{ylabel} vs Steps — Fase 3 Gigante: Q-Former r=128 (10%)",
    ...
    for i, (name, df) in enumerate(histories.items()):
        ...
        ax.set_title(
            f"Train Loss vs Val Loss — {name} (Fase 3 Gigante - r=128)",
            fontsize=13, fontweight="bold",
        )
