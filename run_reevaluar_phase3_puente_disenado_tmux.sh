#!/bin/bash
###############################################################################
# run_reevaluar_phase3_puente_disenado_tmux.sh
###############################################################################
# Re-evaluación SUCIO vs LIMPIO sobre el CSV de Phase 3 PUENTE prompt diseñado
# tras pasar por Llama 8b (etapa 2 → etapa 3 del pipeline).
#
# Métricas calculadas (idénticas a Fase 2):
#   • BLEU-4 + ROUGE-L (CPU, fila por fila)
#   • F1-RadGraph (RG_ERbar, batched chunks=256)
#   • Clinical BERTScore (Bio_ClinicalBERT, batched 64)
#
# Input  : phase3_puente_results-50pct/evaluation/predicciones_Phase3_Puente_llmexterno_llama8b.csv
# Output :
#   • predicciones_Phase3_Puente_llmexterno_llama8b_reeval_detalle.csv   (granular por fila × sucio/limpio/delta)
#   • predicciones_Phase3_Puente_llmexterno_llama8b_reeval_resumen.csv   (medias agregadas)
#
# USO:
#   bash run_reevaluar_phase3_puente_disenado_tmux.sh                   # ambas (recomendado)
#   bash run_reevaluar_phase3_puente_disenado_tmux.sh --no-attach       # 2do plano
#   bash run_reevaluar_phase3_puente_disenado_tmux.sh --reattach        # reconectar
#   bash run_reevaluar_phase3_puente_disenado_tmux.sh --limit=200       # smoke test
###############################################################################

set -uo pipefail

WORKDIR="/workspace/tesis-blip2-mimic"
INPUT_CSV="$WORKDIR/phase3_puente_results-50pct/evaluation/predicciones_Phase3_Puente_llmexterno_llama8b.csv"
LOG_FILE="$WORKDIR/phase3_puente_results-50pct/evaluation/reevaluar_disenado_phase3puente.log"
SESSION="reeval_p3_dis"
...
echo "======================================================================"
echo "  RE-EVALUACIÓN SUCIO vs LIMPIO  ·  Phase 3 PUENTE prompt diseñado"
echo "======================================================================"
echo "  Input    : $INPUT_CSV"
echo "  col-sucio : generated_report  (BLIP-2 crudo)"
...
if [ ! -f "$INPUT_CSV" ]; then
    ...
    echo "       $INPUT_CSV"
    echo "Ejecuta primero: bash run_vllm_formatter_tmux.sh --tasks=disenado_phase3puente"
    exit 1
fi
...
echo '║    tail -f phase3_puente_results-50pct/evaluation/reevaluar_disenado_phase3puente.log  ║' && \
echo '║  Para desconectar (sin matar): Ctrl+b  d                           ║' && \
echo '╚════════════════════════════════════════════════════════════════════╝' && \
echo \"[\$(date +%H:%M:%S)] === Re-evaluacion SUCIO vs LIMPIO Phase 3 PUENTE prompt disenado ===\" | tee -a $LOG_FILE && \
PYTHONUNBUFFERED=1 \
