...
# -*- coding: utf-8 -*-
"""
actualizar_comparativos_final_tier1.py
======================================
Genera los CSVs comparativos del EXPERIMENTO FINAL **TIER 1** (RadDINO +
Asclepius-R + Puente Full-FT + cabeza clínica auxiliar CheXpert) para el
PROMPT DISEÑADO, comparando la salida DIRECTA del modelo (cruda) contra la
versión FORMATEADA por el LLM externo (Llama 3.1 8B):

      · final_tier1_disenado_crudo       (modelo TIER 1 raw, prompt diseñado)
      · final_tier1_disenado_formateado  (raw → Llama 3.1 8B)

Salidas (en final_experiment_tier1_results-50pct/):
    1) metricas_comparativas_inferencias_final_tier1.csv
    ...
    2) chexpert_comparativo_final_tier1.csv
        CheXpert por las 14 patologías + general (micro/macro) para AMBAS
        versiones (crudo y formateado). Se computa con la MISMA agregación
        pooled (TP/FP/FN) validada en el resto del proyecto.

Fuentes (todo PRE-COMPUTADO por evaluate_final_experiment_tier1.py; este script
NO recalcula métricas pesadas — solo numpy + pandas):
    REQUERIDAS (crudo):
      • evaluation/resultados_granulares_Exp_Final_Tier1.csv
    REQUERIDAS (formateado):
      • evaluation/resultados_granulares_Exp_Final_Tier1_formateado.csv
...
patologías, mismo micro/macro) → el comparativo TIER 1 es DIRECTAMENTE comparable
con chexpert_comparativo_final.csv y los demás.
...
F_EVAL_DIR = BASE / "final_experiment_tier1_results-50pct" / "evaluation"

F_GRAN_CRUDO      = F_EVAL_DIR / "resultados_granulares_Exp_Final_Tier1.csv"
F_GRAN_FORMATEADO = F_EVAL_DIR / "resultados_granulares_Exp_Final_Tier1_formateado.csv"

OUT_DIR = BASE / "final_experiment_tier1_results-50pct"
OUT_METRICAS = OUT_DIR / "metricas_comparativas_inferencias_final_tier1.csv"
OUT_CHEXPERT = OUT_DIR / "chexpert_comparativo_final_tier1.csv"
...
def _verificar_archivos(paths_dict: dict) -> None:
    ...
    if faltan:
        print("\nERROR: faltan los siguientes archivos pre-requisito (FINAL TIER 1):")
        for k, p in faltan:
            print(f"  • {k:<18s} → {p}")
        print("\nVerifica que la evaluación TIER 1 haya terminado:")
        print("  1) python evaluate_final_experiment_tier1.py --resume")
        print("  2) python vllm_formatter_tier1.py --resume")
        print("  3) python evaluate_final_experiment_tier1.py --eval-formatted-csv "
              ".../predicciones_Exp_Final_Tier1_llmexterno_llama8b.csv")
        sys.exit(1)


# ═════════════════════════════════════════════════════════════════════════════
# 1) metricas_comparativas_inferencias_final_tier1.csv
#    Delta PAREADO por reporte (crudo vs formateado), unido por dicom_id.
# ═════════════════════════════════════════════════════════════════════════════
def construir_metricas() -> pd.DataFrame:
    print("\n" + "=" * 78)
    print(" CSV 1 — metricas_comparativas_inferencias_final_tier1.csv")
    print("=" * 78)
    ...
    if key is not None:
        ...
        # Fallback no pareado (mismo orden de filas).
    ...
    for col, etiqueta in TEXT_METRICS:
        ...
        filas.append({
            "metrica": etiqueta,
            "final_tier1_disenado_crudo":      round(media_c, ROUND),
            "final_tier1_disenado_formateado": round(media_f, ROUND),
    ...
    filas.append({
        "metrica": "n_muestras",
        "final_tier1_disenado_crudo":      n,
        "final_tier1_disenado_formateado": n,
...
# ═════════════════════════════════════════════════════════════════════════════
# 2) chexpert_comparativo_final_tier1.csv  (crudo + formateado)
# ═════════════════════════════════════════════════════════════════════════════
def construir_chexpert() -> pd.DataFrame:
    print("\n" + "=" * 78)
    print(" CSV 2 — chexpert_comparativo_final_tier1.csv")
    print("=" * 78)
    ...
    merged = (
        base
        .merge(_ren(pat_crudo, "final_tier1_disenado_crudo"),      on="categoria", how="left")
        .merge(_ren(pat_fmt,   "final_tier1_disenado_formateado"), on="categoria", how="left")
    )
    ...
    for scope, etiqueta in [("micro", "MICRO (pooled 14 clases)"),
        ...
        filas_gen.append({
            "nivel": "general",
            "categoria": etiqueta,
            "final_tier1_disenado_crudo_P": cp, "final_tier1_disenado_crudo_R": cr, "final_tier1_disenado_crudo_F1": cf,
            "final_tier1_disenado_formateado_P": fp, "final_tier1_disenado_formateado_R": fr, "final_tier1_disenado_formateado_F1": ff,
        })
    ...
    print("\n   [sanity FINAL TIER 1 — recomputado desde granular]")
...
def main() -> int:
    ...
    print("\n" + "=" * 78)
    print(" Comparativos del Experimento FINAL TIER 1 generados:")
