#!/usr/bin/env python3
"""
run_phase3_50pct.py — Fase 3: Escalamiento Final al 50%
=======================================================
Optimización del Q-Former (LoRA) + language_projection con bases LLM+ViT
congeladas (50% MIMIC-CXR). Objetivo: enseñar al modelo a DETENER la generación
(token EOS) para eliminar el "Tail Babbling".

Configuración Finalista (1 corrida)
-----------------------------------
  Config_Final_B : Base B (LLM r=32 + ViT r=8 de Fase 2/50%) + Q-Former LoRA
                   (r=32, α=64) + language_projection (full fine-tuning).

Por cada corrida este script:
  1. Llama a trainer_utils_phase3_50pct.train_single_config_phase3() → entrenamiento aislado.
  2. Guarda adaptador Q-Former + language_projection.pt en
     phase3_results-50pct/checkpoints/best_model_{name}/.
  3. Llama a fase1_inferencia_phase3() → genera predicciones_{name}.csv en el
     Test Set del 50% cargando LLM + ViT + Q-Former + language_projection.
...
Uso
---
    python run_phase3_50pct.py
    python run_phase3_50pct.py --skip-train              # solo inferencia + eval
    python run_phase3_50pct.py --skip-inference          # reutiliza predicciones existentes
    python run_phase3_50pct.py --num-samples 500         # evalúa solo N muestras del test set
...
# ─────────────────────────────────────────────────────────────────────────────
import os
# Silencia el flood de avisos "TOKENIZERS_PARALLELISM ... fork" que el tokenizer
# rápido emite al re-forkearse en cada época (DataLoader multi-worker). El
# paralelismo intra-tokenizer es irrelevante aquí porque la tokenización ocurre
# dentro de los workers del DataLoader, no en el hilo principal.
...
# ── Utilidades de Fase 3 (trainer_utils_phase3_50pct.py) ──────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# 3.  DEFINICIÓN DE LA CONFIGURACIÓN FINALISTA (1 corrida)
# ─────────────────────────────────────────────────────────────────────────────
#
# base_weights_key: "B" → best_model_Config_B1 (LLM r=32 + ViT r=8 de Fase 2/50%)
#
# En el 50% solo existe la base "B" (única corrida ganadora de Fase 2 al 50%).
# El Q-Former usa r=32, alpha=64 (configuración ganadora del 1% y 10%).
#
...
# ─────────────────────────────────────────────────────────────────────────────
# 4.  FASE 1 PHASE-3-AWARE: Inferencia con 3 adaptadores (LLM + ViT + Q-Former)
# ─────────────────────────────────────────────────────────────────────────────
...
def fase1_inferencia_phase3(
    ...
    Diferencia respecto a fases anteriores
    ----------------------------------------
    Los checkpoints de Fase 3 tienen estructura cuádruple:
        DUAL_WEIGHTS[base_key]["llm"]       ← LLM LoRA (Fase 2)
        DUAL_WEIGHTS[base_key]["vit"]       ← ViT LoRA (Fase 2)
        model_dir/qformer/                  ← Q-Former LoRA (Fase 3)
        model_dir/language_projection.pt    ← language_projection (full FT, Fase 3)

    Esta función carga los TRES adaptadores + la capa de proyección antes de
    lanzar la inferencia.

    Args:
        config_name  : Nombre de la config (p. ej. "Config_Final_A").
        model_dir    : Ruta a phase3_results-10pct/checkpoints/best_model_{name}/.
    ...
    langproj_pt = model_dir / "language_projection.pt"

    for subdir, label in [(llm_dir, "LLM"), (vit_dir, "ViT"), (qformer_dir, "Q-Former")]:
        if not subdir.exists():
            raise FileNotFoundError(
                f"Adaptador {label} no encontrado: {subdir}\n"
                "El entrenamiento debe haber concluido antes de llamar a la inferencia."
            )
    if not langproj_pt.exists():
        raise FileNotFoundError(
            f"Pesos de language_projection no encontrados: {langproj_pt}\n"
            "Fase 3 entrena esta capa (full FT); su .pt es obligatorio para inferencia."
        )

    logger.info(f"\n{'='*65}")
    logger.info(f"  INFERENCIA PHASE-3 (50%): {config_name}")
    logger.info(f"  Base: {base_key}")
    logger.info(f"  LLM                 ← {llm_dir}")
    logger.info(f"  ViT                 ← {vit_dir}")
    logger.info(f"  Q-Former            ← {qformer_dir}")
    logger.info(f"  language_projection ← {langproj_pt}")
    logger.info(f"{'='*65}")

    # ── Cargar modelo base + inyectar los TRES adaptadores ───────────────────
    ...
    # ── language_projection (full FT de Fase 3): cargar state_dict ───────────
    #    Esta capa NO está en ningún adaptador LoRA; se entrenó por completo en
    #    Fase 3 y es la bisagra Q-Former→LLM que alinea el "cuándo parar".
    logger.info(f"  Cargando language_projection (full FT) ← {langproj_pt.name}...")
    _lp_state = torch.load(str(langproj_pt), map_location=_ref.device)
    base_model.language_projection.load_state_dict(_lp_state)
    base_model.language_projection = base_model.language_projection.to(
        device=_ref.device, dtype=_ref.dtype
    )

    logger.info(
        f"  Todos los pesos cargados (LLM+ViT+Q-Former+language_projection) → "
        f"device={_ref.device}, dtype={_ref.dtype}"
    )
...
def run_tournament(
    ...
    """
    Itera sobre las 2 configuraciones finalistas en secuencia.
    ...
    logger.info("Cargando datasets del 50% (train + val)...")
    ...
    for i, cfg in enumerate(configs_to_run, 1):
        name = cfg["name"]
        logger.info(f"\n{'#'*70}")
        logger.info(f"  TORNEO FASE 3 (50%) — Config {i}/{total}: {name}")
        logger.info(f"{'#'*70}")
    ...
    logger.info(f"\nTorneo Fase 3 (50%) completado. Resultados en: {OUTPUT_DIR}")
    ...
    logger.info(f"  plots/01_val_loss_phase3_50pct.png  /  02_bleu4_phase3_50pct.png  /  03_rougeL_phase3_50pct.png\n")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Fase 3 — Escalamiento Final al 50%: Q-Former LoRA + language_projection "
            "con bases LLM+ViT congeladas."
        ),
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Por defecto corre las 2 en secuencia."
        ),
    )
    parser.add_argument(
        "--skip-train",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "existentes en phase3_results-50pct/evaluation/."
        ),
    )
    parser.add_argument(
        "--num-samples",
...
def main() -> int:
    ...
    print("\n" + "="*80)
    print("  FASE 3 — ESCALAMIENTO FINAL AL 50%")
    print("  Q-Former LoRA r=32, α=64 + language_projection (full FT)")
    print("  Objetivo: token EOS → eliminar Tail Babbling")
    print("="*80)
    print(f"  Configuraciones: {len(configs_to_run)}")
    print(f"  Dataset: 50% MIMIC-CXR (escalamiento final)")
