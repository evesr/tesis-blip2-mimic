#!/usr/bin/env python3
"""
run_final_experiment_tier1.py — Lanzador EXPERIMENTO FINAL + TIER 1 (pérdida clínica)
=====================================================================================
RÉPLICA de run_final_experiment.py (el ORIGINAL NO se modifica) que entrena la
MISMA arquitectura del Experimento FINAL añadiendo la pérdida clínica auxiliar
CheXpert (TIER 1): L_total = L_LM + λ·L_clin. Ver trainer_utils_final_experiment_tier1.py.

Entry point que ENTRENA la arquitectura del Experimento FINAL construida por
`trainer_utils_final_experiment_tier1.build_final_experiment_model`:
...
# Constructor + helpers del Experimento FINAL + TIER 1
...
# ─────────────────────────────────────────────────────────────────────────────
# 3-bis.  TIER 1 — Collator con etiquetas CheXpert + Trainer multitarea
# ─────────────────────────────────────────────────────────────────────────────

class DataCollatorWithCheXpert:
    """
    Envuelve a DataCollatorForSeq2Seq: extrae las claves chexpert_labels/mask
    ANTES de delegar (para que el collator base NUNCA las vea) y luego las apila
    en tensores (B,14) y las re-adjunta al batch. Robusto y sin tocar el original.
    """
    def __init__(self, base):
        self.base = base

    def __call__(self, features):
        has = bool(features) and ("chexpert_labels" in features[0])
        clin_l = clin_m = None
        if has:
            clin_l = [f.pop("chexpert_labels") for f in features]
            clin_m = [f.pop("chexpert_mask") for f in features]
        batch = self.base(features)
        if has:
            batch["chexpert_labels"] = torch.stack(
                [torch.as_tensor(x, dtype=torch.float32) for x in clin_l])
            batch["chexpert_mask"] = torch.stack(
                [torch.as_tensor(x, dtype=torch.float32) for x in clin_m])
        return batch


class FinalExperimentTier1Trainer(FinalExperimentTrainer):
    """
    Trainer multitarea (TIER 1): L_total = L_LM + λ · L_clin.

    • L_clin: BCE multietiqueta (14 clases CheXpert) sobre una cabeza lineal
      aplicada al pooling de los 32 query tokens del Q-Former.
    • Los query tokens se capturan con un FORWARD HOOK sobre model.qformer: se
      reutiliza EXACTAMENTE la representación puente que el LLM ya computa, por lo
      que NO hay ningún forward/backward extra (coste temporal ≈ 0).
    • La misma pérdida combinada se usa en validación → eval_loss refleja el
      objetivo real (lenguaje + clínica). Así metric_for_best_model="eval_loss"
      (SIN cambio de hiperparámetro) selecciona el checkpoint mejor en la tarea
      multiobjetivo, en vez del que mejor imita el corpus.
    """
    def __init__(self, *args, lambda_clin: float = 0.5, **kwargs):
        super().__init__(*args, **kwargs)
        self.lambda_clin = float(lambda_clin)
        self._qformer_feat = None
        self._micro_step = 0
        # Hook sobre el Q-Former (submódulo base, no envuelto por accelerate).
        self.model.qformer.register_forward_hook(self._capture_qformer)
        print(f"   [tier1/trainer] pérdida clínica auxiliar ACTIVA · λ={self.lambda_clin} "
              f"· hook Q-Former registrado (0 forward extra)")

    def _capture_qformer(self, module, inputs, output):
        # output[0] = last_hidden_state (B, 32, 768) — mismo tensor que consume
        # language_projection → el gradiente clínico llega al puente completo.
        if isinstance(output, (tuple, list)):
            self._qformer_feat = output[0]
        else:
            self._qformer_feat = getattr(output, "last_hidden_state", None)

    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        clin_labels = inputs.pop("chexpert_labels", None)
        clin_mask   = inputs.pop("chexpert_mask", None)
        ...
        clin_loss = None
        if clin_labels is not None and self._qformer_feat is not None:
            pooled = self._qformer_feat.mean(dim=1)                 # (B, 768)
            head   = self.model.chexpert_aux_head
            logits = head(pooled.to(head.weight.dtype)).float()     # (B, 14)
            tgt    = clin_labels.float()
            msk    = clin_mask.float()
            bce = F.binary_cross_entropy_with_logits(logits, tgt, reduction="none")
            denom = msk.sum().clamp_min(1.0)
            clin_loss = (bce * msk).sum() / denom
            total = lm_loss + self.lambda_clin * clin_loss

        # Log throttled (sin spamear): 1 vez por logging_steps optimizer-steps.
        self._micro_step += 1
        period = max(self.args.logging_steps, 1) * max(self.args.gradient_accumulation_steps, 1)
        if clin_loss is not None and self._micro_step % period == 0:
            print(f"      [tier1] step≈{self.state.global_step}  "
                  f"lm={float(lm_loss.detach()):.4f}  "
                  f"clin={float(clin_loss.detach()):.4f}  λ={self.lambda_clin}")
...
def train_final_experiment(
    ...
    training_args = TrainingArguments(
        ...
        per_device_eval_batch_size=24,   # 24 (=Exp2/Exp3/Final, probado estable 6h). NO subir: eval_batch=64 causó CUDA OOM — los logits (batch×seq×vocab 32000) que cross_entropy sube a fp32 pidieron 4.39 GiB. La aceleración del eval se logra con el SUBCONJUNTO de val (ver main), no con más batch.
    ...
    # TIER 1: se envuelve el collator para transportar las etiquetas CheXpert.
    data_collator = DataCollatorWithCheXpert(
        DataCollatorForSeq2Seq(tokenizer=processor.tokenizer, padding=True)
    )
    ...
    trainer = FinalExperimentTier1Trainer(
        model=model,
        args=training_args,
        ...
        lambda_clin=TRAINING_CONFIG["lambda_clin"],
    )
...
def main() -> int:
    ...
    if args.smoke:
        ...
        # ── TIER 1 · OPT DE TIEMPO: subconjunto FIJO de val para el eval en-bucle ──
        # El eval en-bucle (cada eval_steps) solo sirve para early-stopping y para
        # seleccionar el mejor checkpoint por val_loss. NO influye en el gradiente,
        # los pesos, el lr, el batch, las épocas ni el objetivo → NO afecta la
        # CALIDAD del modelo entrenado. Las MÉTRICAS REALES (CheXpert/BLEU/RadGraph)
        # se calculan aparte sobre el TEST completo por los scripts de evaluación.
        # Un subconjunto aleatorio FIJO (seed=42) de 10.000/38.989 muestras da un
        # estimado de val_loss estadísticamente equivalente (error estándar ínfimo)
        # y recorta el eval de ~25 min a ~5 min → la "vuelta" total baja de ~51 a
        # ~30 min. Control: TIER1_EVAL_SUBSET (0 = usar val completo).
        _eval_subset = int(os.environ.get("TIER1_EVAL_SUBSET", "10000"))
        if 0 < _eval_subset < len(val_ds.data):
            _n_full = len(val_ds.data)
            val_ds.data = val_ds.data.sample(
                n=_eval_subset, random_state=42
            ).reset_index(drop=True)
            print(f"   [TIER 1 opt] Eval en-bucle sobre subconjunto FIJO de val: "
                  f"{len(val_ds.data):,}/{_n_full:,} (seed=42). "
                  f"Acelera el eval sin afectar train ni el modelo final; "
                  f"las métricas finales usan el TEST completo.")
    ...
    # TIER 1: las 4 ramas de evaluación (evaluate_final_experiment.py, etc.) apuntan
    # por defecto a las rutas del experimento final ORIGINAL. Para NO evaluar el
    # checkpoint equivocado ni duplicar horas de cómputo, NO se lanzan de forma
    # automática. El checkpoint de Tier 1 es compatible con build_final_experiment_model
    # (la cabeza auxiliar se ignora con strict=False), así que puede evaluarse con los
    # scripts existentes apuntándolos a este OUTPUT_DIR/checkpoint.
    if args.smoke:
        print("   [SMOKE] Evaluación post-entrenamiento omitida en modo smoke.")
    else:
        print("\n" + "=" * 80)
        print("  TIER 1 — entrenamiento terminado. Evaluación NO automática.")
        print("  Checkpoint entrenable (puente + cabeza clínica) en:")
        print(f"    {CHECKPOINTS_DIR / f'best_model_{RUN_NAME}'}")
        print("  Para evaluar, reutiliza los scripts del final apuntando a este run.")
        print("=" * 80)
