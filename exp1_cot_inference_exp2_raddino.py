#!/usr/bin/env python3
"""
exp1_cot_inference_exp2_raddino.py — Experimento 1 (CoT) sobre EXPERIMENTO 2
================================================================================

Misma idea que exp1_cot_inference_phase3_puente.py, pero ejecutado sobre el
checkpoint del EXPERIMENTO 2 (RadDINO + Puente Full-FT). El modelo se reconstruye
con build_experiment_2_model() + exp2_state.pt (strict=False); el resto del
pipeline (prompt CoT, GEN_PARAMS y las 5 métricas) es IDÉNTICO al de Fase 3
PUENTE, para que los resultados sean DIRECTAMENTE comparables.

Pipeline
--------
FASE 1 — Inferencia visual + generación con prompt Chain-of-Thought (PROMPT NUEVO)
    • Modelo Experimento 2 (cargar_modelo_exp2_raddino → RadDINO + Puente)
    ...
    • Test Set 50% (mismas imágenes/csv que evaluate_exp2_raddino)
    • Reanudable: si el CSV crudo ya contiene filas, salta dicom_id ya hechos
    • Output: exp2_raddino_results-50pct/exp1_cot/
              exp1_raw_inference_exp2_raddino.csv
...
FASE 2 — Evaluación granular fila por fila (las 5 métricas requeridas)
    ...
    • Outputs:
        - resultados_granulares_Exp2_RadDINO_CoT.csv   (granular, 1 fila/reporte)
        - metricas_globales_Exp2_RadDINO_CoT.csv       (medias agregadas)

Uso
---
    python exp1_cot_inference_exp2_raddino.py
    python exp1_cot_inference_exp2_raddino.py --num-samples 200    # smoke
    python exp1_cot_inference_exp2_raddino.py --skip-inference     # reusa raw
    python exp1_cot_inference_exp2_raddino.py --resume             # reanuda
    python exp1_cot_inference_exp2_raddino.py --no-bertscore       # debug rápido
...
# (carga del modelo Experimento 2 + métricas idénticas a la evaluación con
#  prompt diseñado). evaluate_exp2_raddino re-exporta los helpers de métrica de
#  evaluate_phase3_puente, por lo que la lógica es EXACTAMENTE la misma.
from evaluate_exp2_raddino import (
    cargar_modelo_exp2_raddino,
    _bleu4,
    _rouge_l,
    _radgraph_f1,
    _chexpert_pred_cols,
...
# ── Helpers de Clinical BERTScore (los mismos que evaluate_exp2_raddino) ──
...
# ── Rutas del Experimento 2 ───────────────────────────────────────────────────
from trainer_utils_exp2_raddino import OUTPUT_DIR
...
# ─────────────────────────────────────────────────────────────────────────────
# 3. Constantes propias del experimento (paths exclusivos del CoT Exp2)
# ─────────────────────────────────────────────────────────────────────────────
...
# en vllm_formatter.TASKS["nuevo_exp2raddino"].in_csv. Si lo cambias, actualiza
# también vllm_formatter.py.
RAW_CSV:      Path = EXP1_DIR / "exp1_raw_inference_exp2_raddino.csv"
GRANULAR_CSV: Path = EXP1_DIR / "resultados_granulares_Exp2_RadDINO_CoT.csv"
GLOBAL_CSV:   Path = EXP1_DIR / "metricas_globales_Exp2_RadDINO_CoT.csv"

CONFIG_NAME: str = "Exp2_RadDINO_CoT"
...
def fase_inferencia_cot_exp2_raddino(
    model,
    processor,
    device,
    ...
    Réplica de exp1_cot_inference_phase3_puente.fase_inferencia_cot_phase3_puente
    con los paths del Experimento 2.
    """
    ...
    t0 = time.time()
    for idx in tqdm(range(n_eval), desc="  Inferencia CoT (Exp2 RadDINO)", unit="img"):
...
def fase2_evaluacion_cot(
    ...
    Evaluación fila por fila SOBRE el RAW_CSV de inferencia CoT:
        ...
        4. CheXpert P/R/F1 + match por enfermedad (lógica idéntica a Fase 2/3)
        5. Clinical BERTScore (Bio_ClinicalBERT) batched al final

    Réplica funcional de exp1_cot_inference_phase3_puente.fase2_evaluacion_cot.
    """
    logger.info(f"\n{SEP}")
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=("Experimento 1 (CoT) sobre Experimento 2 (RadDINO): "
    ...
    p.add_argument("--exp2-state", type=str, default=str(EXP2_STATE),
                   help="exp2_state.pt entrenado en el Experimento 2 "
    ...
    p.add_argument("--skip-inference", action="store_true",
                   help="Reutilizar exp1_raw_inference_exp2_raddino.csv y "
                        "solo (re)evaluar.")
...
def main() -> int:
    ...
    print("\n" + "=" * 80)
    print("  EXPERIMENTO 1 — CHAIN-OF-THOUGHT  ·  EXPERIMENTO 2 (RadDINO + Puente)")
    ...
    if args.skip_inference and RAW_CSV.exists():
        ...
        fase_inferencia_cot_exp2_raddino(
    ...
    if args.skip_evaluation:
        ...
        logger.info("  Próximo paso sugerido: "
                    "bash run_vllm_formatter_tmux.sh --tasks=nuevo_exp2raddino")
        return 0
    ...
    print("\n" + "=" * 80)
    print("  EXP1 (CoT)  ·  EXPERIMENTO 2 (RadDINO)  ·  COMPLETADO")
    print("=" * 80)
    print(f"  Raw          : {RAW_CSV}")
    print(f"  Granular     : {GRANULAR_CSV}")
    print(f"  Global       : {GLOBAL_CSV}")
    print("\n  Siguiente   : bash run_vllm_formatter_tmux.sh --tasks=nuevo_exp2raddino")
    print("=" * 80 + "\n")
    return 0
