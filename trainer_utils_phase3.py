#!/usr/bin/env python3
"""
trainer_utils_phase3.py — Utilidades de entrenamiento para Fase 3
==================================================================
Optimización Semántica del Q-Former con bases LLM+ViT congeladas (1% MIMIC-CXR).
...
2. Phase3AuditCallback
       ...
       solo si val_loss mejora. Registra history_phase3_{name}.csv y
       audit_phase3_{name}.txt.
...
# Fase 3 trabaja con el 1% del dataset (mini-torneo de exploración rápida)
...
DUAL_WEIGHTS: Dict[str, Dict[str, Path]] = {
    "A": {
        "llm": BASE_DIR / "phase2_results-10pct" / "checkpoints" / "best_model_Config_A2" / "llm",
        "vit": BASE_DIR / "phase2_results-10pct" / "checkpoints" / "best_model_Config_A2" / "vit",
    },
    "B": {
...
# Directorio raíz de salida para toda la Fase 3
OUTPUT_DIR      = BASE_DIR / "phase3_results"
...
TRAINING_CONFIG: Dict = {
    # Learning rate fijo (sin scheduler dinámico para esta fase de torneo)
    "lr":                      1e-4,
...
def inject_qformer_lora(
    ...
    Estrategia de inyección (Fase 3)
    ---------------------------------
    ...
    Args:
        ...
        qformer_r       : Rango LoRA para Q-Former (16 o 32).
        qformer_alpha   : Escalado LoRA para Q-Former (32 o 64).
    ...
    print(f"   [qformer_lora] Q-Former entrenables : {n_qformer:,} params")
...
class Phase3AuditCallback(TrainerCallback):
    """
    Callback ejecutado después de cada evaluate() del Trainer (Fase 3).
    ...
    def __init__(
        ...
        self.audit_path.write_text(
            f"# Audit Log — Fase 3 — {config_name}\n"
    ...
    def on_evaluate(
        ...
        **kwargs,
        ...
        # — Guardar adaptador Q-Former si val_loss mejoró —
...
def train_single_config_phase3(
    ...
    """
    Entrena una configuración de Fase 3 de forma completamente aislada.

    Flujo
    -----
    1. Llama a inject_qformer_lora() para construir el modelo con Q-Former LoRA.
    2. Configura optimizador AdamW simple (lr fijo 1e-4).
    ...
    # ── 2. Optimizador simple (solo params del Q-Former) ─────────────────────
    ...
    print(f"   [optimizer] lr={TRAINING_CONFIG['lr']:.0e}  (fijo, sin scheduler)")
    ...
    training_args = TrainingArguments(
        ...
        # Sin scheduler dinámico para esta fase de torneo
...
def plot_comparative_curves(
    ...
    specs = [
        ("Val_Loss", "Val Loss",  "01_val_loss_phase3.png"),
        ("BLEU-4",   "BLEU-4",   "02_bleu4_phase3.png"),
        ("ROUGE-L",  "ROUGE-L",  "03_rougeL_phase3.png"),
    ]
    ...
    for col, ylabel, filename in specs:
        ...
        ax.set_title(
            f"{ylabel} vs Steps — Fase 3: Optimización Semántica Q-Former",
    ...
    for i, (name, df) in enumerate(histories.items()):
        ...
        ax.set_title(
            f"Train Loss vs Val Loss — {name} (Fase 3)",
            fontsize=13, fontweight="bold",
        )
...
def load_phase3_datasets(processor) -> Tuple[MimicCXRDataset, MimicCXRDataset]:
    """
    Carga los datasets de train y val del 1% de MIMIC-CXR (Fase 3).
    ...
    print("   [datasets] Cargando Train (1%)...")
    train_ds = MimicCXRDataset(csv_path=TRAIN_CSV_1PCT, **shared_kwargs)

    print("   [datasets] Cargando Val   (1%)...")
    val_ds   = MimicCXRDataset(csv_path=VAL_CSV_1PCT,   **shared_kwargs)
