#!/usr/bin/env python3
"""
exp1_cot_inference_phase3_puente.py — Experimento 1 (CoT) sobre FASE 3 PUENTE
================================================================================

Misma idea que exp1_cot_inference.py (Fase 2 dual-LoRA), pero ejecutado sobre el
checkpoint de FASE 3 PUENTE: ViT y LLM LoRA fusionados PERMANENTEMENTE +
puente_state.pt (qformer + language_projection + query_tokens) cargados con
strict=False.

Pipeline
--------
FASE 1 — Inferencia visual + generación con prompt Chain-of-Thought
    • Modelo Phase 3 PUENTE (4 pasos de carga, ver evaluate_phase3_puente)
    ...
    • Test Set 50% (mismas imágenes/csv que evaluate_phase3_puente)
    • Reanudable: si el CSV crudo ya contiene filas, salta dicom_id ya hechos
    • Output: phase3_puente_results-50pct/exp1_cot/
              exp1_raw_inference_phase3_puente.csv
...
FASE 2 — Evaluación granular fila por fila (las 5 métricas requeridas)
    ...
    • BLEU-4 / ROUGE-L → fila por fila vs reference_report
    • F1-RadGraph      → RG_ERbar (radgraph-xl, idx 2)
    • Clinical BERTScore → Bio_ClinicalBERT (12 capas, sin baseline) batched
    • Outputs:
        - resultados_granulares_Phase3_Puente_CoT.csv   (granular, 1 fila/reporte)
        - metricas_globales_Phase3_Puente_CoT.csv       (medias agregadas)

Uso
---
    python exp1_cot_inference_phase3_puente.py
    python exp1_cot_inference_phase3_puente.py --num-samples 200    # smoke
    python exp1_cot_inference_phase3_puente.py --skip-inference     # reusa raw
    python exp1_cot_inference_phase3_puente.py --resume             # reanuda
    python exp1_cot_inference_phase3_puente.py --no-bertscore       # debug rápido
...
# (carga del modelo + métricas idénticas a la evaluación con prompt diseñado)
from evaluate_phase3_puente import (
    cargar_modelo_phase3_puente,
    _bleu4,
    _rouge_l,
    _radgraph_f1,
    _chexpert_pred_cols,
...
# ── Reutilización del prompt Chain-of-Thought y los gen-params del experimento
...
# ── Helpers de Clinical BERTScore (los mismos que evaluate_phase3_puente) ──
...
# ─────────────────────────────────────────────────────────────────────────────
# 3. Constantes propias del experimento (paths exclusivos del CoT Phase 3)
# ─────────────────────────────────────────────────────────────────────────────
BASE_DIR: Path = config.paths.base_dir

PHASE3_PUENTE_DIR: Path = BASE_DIR / "phase3_puente_results-50pct"
EXP1_DIR:          Path = PHASE3_PUENTE_DIR / "exp1_cot"

# IMPORTANTE: el nombre EXACTO de RAW_CSV debe coincidir con el path declarado
# en vllm_formatter.TASKS["nuevo_phase3puente"].in_csv. Si lo cambias, actualiza
# también vllm_formatter.py.
RAW_CSV:           Path = EXP1_DIR / "exp1_raw_inference_phase3_puente.csv"
GRANULAR_CSV:      Path = EXP1_DIR / "resultados_granulares_Phase3_Puente_CoT.csv"
GLOBAL_CSV:        Path = EXP1_DIR / "metricas_globales_Phase3_Puente_CoT.csv"

CONFIG_NAME: str = "Phase3_Puente_CoT"
...
def fase_inferencia_cot_phase3_puente(
    model,
    processor,
    device,
    ...
    ya generados (robusto frente a pausas de pod).

    Réplica de exp1_cot_inference.fase_inferencia_cot pero (a) con paths del
    experimento Phase 3 PUENTE y (b) sin asumir que las columnas del Test Set
    incluyan ViewPosition (se respeta el mismo fallback a 'Unknown').
    """
    ...
    t0 = time.time()
    for idx in tqdm(range(n_eval), desc="  Inferencia CoT (Phase 3 PUENTE)", unit="img"):
...
def fase2_evaluacion_cot(
    ...
    """
    Evaluación fila por fila SOBRE el RAW_CSV de inferencia CoT:

        1. CheXbert sobre generated_report → pred_chexpert_*
        2. Merge GT oficial (study_id)
        3. BLEU-4 / ROUGE-L / F1-RadGraph fila por fila
        4. CheXpert P/R/F1 + match por clase (lógica idéntica a Fase 2/3)
        5. Clinical BERTScore (Bio_ClinicalBERT) batched al final

    Réplica funcional de evaluate_phase3_puente.fase2_evaluacion ajustada para
    el CSV crudo del CoT (que ya tiene generated_report listo para etiquetar).
    """
    logger.info(f"\n{SEP}")
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df),
        ...
        # CheXpert: comparación GT vs Pred (P/R/F1 micro + match por clase)
        if has_chexpert_gt:
            try:
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=("Experimento 1 (CoT) sobre Phase 3 PUENTE: inferencia con "
                     "Chain-of-Thought + evaluación granular (BLEU/ROUGE/RadGraph"
                     "/CheXpert/Clinical BERTScore)."),
    ...
    p.add_argument("--batch-size", type=int, default=16,
                   help="Tamaño de lote de inferencia "
                        "(beam=3, max_new=512 → 16 óptimo en L40S 45GB).")
    ...
    p.add_argument("--model-vit",  type=str, default=str(PHASE2_VIT_DIR),
                   help="Adaptador LoRA ViT de Fase 2 (se fusiona).")
    p.add_argument("--model-llm",  type=str, default=str(PHASE2_LLM_DIR),
                   help="Adaptador LoRA LLM de Fase 2 (se fusiona).")
    p.add_argument("--puente-state", type=str, default=str(PHASE3_PUENTE_STATE),
                   help="puente_state.pt (qformer + language_projection + query_tokens).")
    ...
    p.add_argument("--skip-inference", action="store_true",
                   help="Reutilizar exp1_raw_inference_phase3_puente.csv y "
                        "solo (re)evaluar.")
...
def main() -> int:
    ...
    print("\n" + "=" * 80)
    print("  EXPERIMENTO 1 — CHAIN-OF-THOUGHT  ·  FASE 3 PUENTE (50%)")
    ...
    if args.skip_inference and RAW_CSV.exists():
        ...
        model, device = cargar_modelo_phase3_puente(
            ruta_lora_vit     = Path(args.model_vit),
            ruta_lora_llm     = Path(args.model_llm),
            ruta_puente_state = Path(args.puente_state),
        )
        fase_inferencia_cot_phase3_puente(
            model, processor, device,
            num_samples = args.num_samples,
            batch_size  = args.batch_size,
            resume      = args.resume,
        )
        # Liberar BLIP-2 antes de cargar CheXbert + RadGraph + ClinBERT
        del model
        gc.collect()
        torch.cuda.empty_cache()
    ...
    if args.skip_evaluation:
        ...
        logger.info("  Próximo paso sugerido: "
                    "bash run_vllm_formatter_tmux.sh --tasks=nuevo_phase3puente")
        return 0
    ...
    print("\n" + "=" * 80)
    print("  EXP1 (CoT)  ·  FASE 3 PUENTE  ·  COMPLETADO")
    print("=" * 80)
    print(f"  Raw          : {RAW_CSV}")
    print(f"  Granular     : {GRANULAR_CSV}")
    print(f"  Global       : {GLOBAL_CSV}")
    print("\n  Siguiente   : bash run_vllm_formatter_tmux.sh --tasks=nuevo_phase3puente")
    print("=" * 80 + "\n")
    return 0
