#!/bin/bash
###############################################################################
# run_exp1_cot_exp2_raddino_tmux.sh
###############################################################################
# Inferencia CoT (Chain-of-Thought) sobre EXPERIMENTO 2 (RadDINO + Puente) +
# evaluación granular (BLEU-4 / ROUGE-L / F1-RadGraph / CheXpert / Clinical
# BERTScore).
#
...
#   • exp2_state.pt → load_state_dict(strict=False)
#       (vision_model LoRA+proyector + qformer + language_projection +
#        query_tokens, ~108.5 M params)
#
# Generación
#   • num_beams=3, max_new_tokens=512, batch=16  (≈30 GB VRAM en L40S)
#
# Outputs (en exp2_raddino_results-50pct/exp1_cot/)
#   • exp1_raw_inference_exp2_raddino.csv          (inferencia cruda)
#   • resultados_granulares_Exp2_RadDINO_CoT.csv   (5 métricas granulares)
#   • metricas_globales_Exp2_RadDINO_CoT.csv       (resumen)
#
# USO típico
#   bash run_exp1_cot_exp2_raddino_tmux.sh                    # full pipeline
#   bash run_exp1_cot_exp2_raddino_tmux.sh --no-attach        # 2do plano
#   bash run_exp1_cot_exp2_raddino_tmux.sh --reattach
#   bash run_exp1_cot_exp2_raddino_tmux.sh --num-samples=200  # smoke
#   bash run_exp1_cot_exp2_raddino_tmux.sh --skip-evaluation  # solo inferencia
#   bash run_exp1_cot_exp2_raddino_tmux.sh --skip-inference   # solo evaluación
#   bash run_exp1_cot_exp2_raddino_tmux.sh --resume           # reanudar
#   bash run_exp1_cot_exp2_raddino_tmux.sh --no-bertscore     # debug rápido
#
...
LOG_FILE="$WORKDIR/exp2_raddino_results-50pct/exp1_cot/exp1_cot_exp2_raddino.log"
SESSION="exp1_cot_exp2"
...
echo "======================================================================"
echo "  EXP1 CoT  ·  EXPERIMENTO 2 (RadDINO)  ·  Inferencia + Evaluación"
echo "======================================================================"
...
# Validaciones de pre-requisitos (paths del modelo Experimento 2).
...
for p in "$PHASE2_BEST/llm/adapter_config.json" \
         "$EXP2_PT" \
         "$WORKDIR/exp1_cot_inference_exp2_raddino.py"; do
    if [ ! -e "$p" ]; then
...
echo '║    tail -f exp2_raddino_results-50pct/exp1_cot/exp1_cot_exp2_raddino.log ║' && \
echo '║  Para desconectar (sin matar): Ctrl+b  d                           ║' && \
echo '╚════════════════════════════════════════════════════════════════════╝' && \
echo \"[\$(date +%H:%M:%S)] === EXP1 CoT Experimento 2 (RadDINO) inicia ===\" | tee -a $LOG_FILE && \
PYTHONUNBUFFERED=1 \
TOKENIZERS_PARALLELISM=false \
PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
python -u exp1_cot_inference_exp2_raddino.py ${EXTRA_ARGS[*]:-} 2>&1 | tee -a $LOG_FILE; \
