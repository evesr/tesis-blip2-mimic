#!/bin/bash
###############################################################################
# run_reevaluar_exp2_raddino_cot_tmux.sh
###############################################################################
# Re-evaluación SUCIO vs LIMPIO sobre el CSV CoT del Experimento 2 (RadDINO)
# tras Llama 8b (etapa 4 del pipeline).
#
# Métricas calculadas (idénticas a la etapa diseñado):
...
#
# Input  : exp2_raddino_results-50pct/exp1_cot/exp1_final_formatted_exp2_raddino_llama8b.csv
# Output :
#   • exp1_final_formatted_exp2_raddino_llama8b_reeval_detalle.csv
#   • exp1_final_formatted_exp2_raddino_llama8b_reeval_resumen.csv
#
# USO:
#   bash run_reevaluar_exp2_raddino_cot_tmux.sh                   # ambas
#   bash run_reevaluar_exp2_raddino_cot_tmux.sh --no-attach       # 2do plano
#   bash run_reevaluar_exp2_raddino_cot_tmux.sh --reattach        # reconectar
#   bash run_reevaluar_exp2_raddino_cot_tmux.sh --limit=200       # smoke
#
# Tiempo estimado: ~1h (mismo flujo y volumen que la etapa diseñado).
###############################################################################

set -uo pipefail

WORKDIR="/workspace/tesis-blip2-mimic"
INPUT_CSV="$WORKDIR/exp2_raddino_results-50pct/exp1_cot/exp1_final_formatted_exp2_raddino_llama8b.csv"
LOG_FILE="$WORKDIR/exp2_raddino_results-50pct/exp1_cot/reevaluar_cot_exp2raddino.log"
SESSION="reeval_exp2_cot"
...
echo "======================================================================"
echo "  RE-EVALUACIÓN SUCIO vs LIMPIO  ·  Experimento 2 (RadDINO) prompt CoT"
echo "======================================================================"
echo "  Input    : $INPUT_CSV"
echo "  col-sucio : generated_report  (BLIP-2/RadDINO CoT crudo)"
...
if [ ! -f "$INPUT_CSV" ]; then
    ...
    echo "       $INPUT_CSV"
    echo "Ejecuta primero: bash run_vllm_formatter_tmux.sh --tasks=nuevo_exp2raddino"
    exit 1
fi
...
echo '║    tail -f exp2_raddino_results-50pct/exp1_cot/reevaluar_cot_exp2raddino.log ║' && \
echo '║  Para desconectar (sin matar): Ctrl+b  d                           ║' && \
echo '╚════════════════════════════════════════════════════════════════════╝' && \
echo \"[\$(date +%H:%M:%S)] === Re-evaluacion SUCIO vs LIMPIO Exp2 RadDINO prompt CoT ===\" | tee -a $LOG_FILE && \
PYTHONUNBUFFERED=1 \
