#!/usr/bin/env python3
"""
trainer_utils_phase3_coadapt.py — Utilidades de entrenamiento para Fase 3 Co-adaptación
=========================================================================================
Estudio de Ablación — Paso 2: Co-adaptación de Espacios Latentes.

Las últimas dos capas del ViT (encoder.layers.37 y encoder.layers.38) se
descongelan junto con el Q-Former LoRA (r=128), permitiendo que el extractor
visual se co-adapte con el puente de proyección para cerrar la brecha modal.

Responsabilidades
-----------------
1. inject_qformer_lora()
       • Carga adaptadores LLM y ViT de Fase 2 y hace merge_and_unload().
       • Congela COMPLETAMENTE el modelo fusionado.
       • Descongela QUIRÚRGICAMENTE encoder.layers.37 y encoder.layers.38 del ViT.
       • Inyecta LoRA SOLO en el Q-Former (target_modules=["query", "value"]).

2. Phase3AuditCallback
       Guarda adaptador Q-Former en checkpoints/best_model_{name}/qformer/
       y pesos co-adaptados del ViT como vit_coadapted.pt en best_model_{name}/
       SOLO si val_loss mejora. Registra history_phase3_coadapt_{name}.csv y
       audit_phase3_coadapt_{name}.txt.
...
# Fase Co-adaptación trabaja con el 10% del dataset (escalamiento final)
...
# Solo Base B (best_model_Config_B1) — línea base para la co-adaptación
...
# Directorio raíz de salida para la Fase Co-adaptación
OUTPUT_DIR      = BASE_DIR / "phase3_results-coadapt"
...
# ─────────────────────────────────────────────────────────────────────────────
# 1.  INYECCIÓN Q-FORMER LoRA + DESCONGELAMIENTO QUIRÚRGICO ViT
# ─────────────────────────────────────────────────────────────────────────────
...
def inject_qformer_lora(
    ...
    """
    Construye el modelo BLIP-2 con adaptador LoRA en Q-Former y descongelamiento
    quirúrgico de las capas 37-38 del ViT (co-adaptación de espacios latentes).

    Estrategia de inyección (Fase Co-adaptación)
    --------------------------------------------
    ...
    5. [CO-ADAPTACIÓN] Descongela QUIRÚRGICAMENTE encoder.layers.37 y
       encoder.layers.38 del ViT (las dos capas más cercanas al Q-Former).
    6. Inyecta LoRA nuevo en Q-Former con:
    ...
    Args:
        ...
        qformer_r       : Rango LoRA para Q-Former (128 en config Co-adaptación).
        qformer_alpha   : Escalado LoRA para Q-Former (256 en config Co-adaptación).
    ...
    print(f"   [coadapt] Cargando modelo base BLIP-2 (bfloat16, device_map=auto)...")
    ...
    print(f"   [coadapt] Cargando LLM LoRA desde {llm_weights_dir.parent.name}/{llm_weights_dir.name}...")
    llm_peft = PeftModel.from_pretrained(
        base_model.language_model,
        str(llm_weights_dir),
        is_trainable=False,
    )
    print(f"   [coadapt] Fusionando LLM LoRA (merge_and_unload)...")
    base_model.language_model = llm_peft.merge_and_unload()

    # ── ViT: cargar adaptador y fusionar ─────────────────────────────────────
    print(f"   [coadapt] Cargando ViT LoRA desde {vit_weights_dir.parent.name}/{vit_weights_dir.name}...")
    vit_peft = PeftModel.from_pretrained(
        base_model.vision_model,
        str(vit_weights_dir),
        is_trainable=False,
    )
    print(f"   [coadapt] Fusionando ViT LoRA (merge_and_unload)...")
    ...
    print(f"   [coadapt] Congelando TODOS los parámetros del modelo fusionado...")
    for param in base_model.parameters():
        param.requires_grad = False

    # ── Descongelamiento Quirúrgico del ViT (Co-adaptación) ─────────────────
    # En EVA-CLIP (ViT de BLIP-2), las capas son 'encoder.layers.X'
    # Descongelamos las capas 37 y 38 (las más cercanas al Q-Former)
    # para permitir que el extractor visual se co-adapte al espacio Q-Former
    print(f"   [coadapt] Descongelamiento quirúrgico: ViT encoder.layers.37 y encoder.layers.38...")
    for name, param in base_model.vision_model.named_parameters():
        if "encoder.layers.37" in name or "encoder.layers.38" in name:
    ...
    print(f"   [coadapt] Inyectando Q-Former LoRA (r={qformer_r}, alpha={qformer_alpha}, "
    ...
    n_vit_coadapt = sum(
        p.numel() for n, p in base_model.vision_model.named_parameters()
        if p.requires_grad
    )
    print(f"   [coadapt] Q-Former entrenables        : {n_qformer:,} params")
    print(f"   [coadapt] ViT co-adapt (layers 37-38) : {n_vit_coadapt:,} params")
    ...
    print(
        f"\n   ┌─── Parámetros del modelo (Fase 3 Co-adaptación: Q-Former LoRA r=128 + ViT) ──┐\n"
        f"   │  Total              : {total:>15,}                                    │\n"
        f"   │  Entrenables        : {trainable:>15,}  ({pct:.4f} %)                    │\n"
        f"   │    └─ Q-Former LoRA : {n_qformer:>15,}                                    │\n"
        f"   │    └─ ViT layers    : {n_vit_coadapt:>15,}  (layers 37-38)                │\n"
        f"   │  Congelados         : {total - trainable:>15,}                                    │\n"
        f"   │    └─ LLM           : {n_llm_frozen:>15,}                                    │\n"
        f"   │    └─ ViT (resto)   : {n_vit_total - n_vit_coadapt:>15,}                                    │\n"
        f"   └───────────────────────────────────────────────────────────────────────────┘\n"
    )
...
class Phase3AuditCallback(TrainerCallback):
    """
    Callback ejecutado después de cada evaluate() del Trainer (Fase Co-adaptación).

    Responsabilidades
    -----------------
    Checkpoint Q-Former LoRA + ViT co-adaptado
        Guarda adaptador Q-Former en best_model_{name}/qformer/
        y los pesos co-adaptados del ViT (layers 37-38) como
        best_model_{name}/vit_coadapted.pt
        SOLO si val_loss mejoró respecto al mejor registrado.

    Historial
        Appends a history_phase3_coadapt_{config_name}.csv:
            Step | Train_Loss | Val_Loss | BLEU-4 | ROUGE-L

    Audit log
        Appends a audit_phase3_coadapt_{config_name}.txt con predicciones
    ...
    def __init__(
        ...
        self.history_path   = output_dir / f"history_phase3_coadapt_{config_name}.csv"
        self.audit_path     = output_dir / f"audit_phase3_coadapt_{config_name}.txt"
        ...
        self.audit_path.write_text(
            f"# Audit Log — Fase 3 Co-adaptación (r=128, ViT layers 37-38, 10%) — {config_name}\n"
    ...
    def on_evaluate(
        ...
        **kwargs,
        ...
        print(
            f"\n   [Coadapt Step {step}] train_loss={train_loss_str}  "
            f"val_loss={val_loss:.4f}  BLEU-4={mean_bleu4:.4f}  ROUGE-L={mean_rougeL:.4f}"
        )

        # — Guardar Q-Former + ViT co-adaptado si val_loss mejoró —
        if val_loss is not None and val_loss < self.best_val_loss:
            ...
            # ViT co-adaptado → best_model_{name}/vit_coadapted.pt
            # Solo guardamos los parámetros del ViT que tienen requires_grad=True
            # (encoder.layers.37 y encoder.layers.38)
            vit_coadapted_state = {
                name: param.data.cpu()
                for name, param in model.vision_model.named_parameters()
                if param.requires_grad
            }
            vit_save = self.best_model_dir / "vit_coadapted.pt"
            torch.save(vit_coadapted_state, str(vit_save))

            print(
                f"   ✓ Checkpoint co-adaptación guardado (val_loss={val_loss:.4f})\n"
                f"     Q-Former      → {qformer_save}\n"
                f"     ViT layers    → {vit_save}  "
                f"({len(vit_coadapted_state)} tensores)"
            )
...
def train_single_config_phase3(
    ...
    """
    Entrena una configuración de Fase Co-adaptación de forma completamente aislada.

    Flujo
    -----
    1. Llama a inject_qformer_lora() para construir el modelo con Q-Former LoRA
       y ViT layers 37-38 descongelados (co-adaptación).
    2. Configura optimizador AdamW simple (lr 1e-4) sobre TODOS los params entrenables
       (Q-Former LoRA + ViT layers 37-38).
    3. Configura TrainingArguments (bf16=True, gradient_checkpointing=True, cosine scheduler).
    ...
    print(f"\n{'='*80}")
    print(f"  FASE 3 CO-ADAPTACIÓN (10%) — Config {config_num}/{total_configs}: {name}")
    ...
    # ── 1. Construir modelo con Q-Former LoRA + ViT co-adaptación ───────────
    ...
    # ── 2. Optimizador (Q-Former LoRA + ViT layers 37-38) ────────────────────
    ...
    if existing_ckpts:
        ...
        print("   [NUEVO] Sin checkpoints previos. Iniciando co-adaptación desde cero.")
        trainer.train()
    ...
    print(
        f"\n   ✓ Co-adaptación completada: steps={result['total_steps']}  "
...
def plot_comparative_curves(
    ...
    las configs del torneo de co-adaptación.

    Lee los archivos history_phase3_coadapt_{name}.csv escritos por
    Phase3AuditCallback.
    ...
    for name in config_names:
        p = output_dir / f"history_phase3_coadapt_{name}.csv"
        if p.exists():
    ...
    specs = [
        ("Val_Loss", "Val Loss",  "01_val_loss_phase3_coadapt.png"),
        ("BLEU-4",   "BLEU-4",   "02_bleu4_phase3_coadapt.png"),
        ("ROUGE-L",  "ROUGE-L",  "03_rougeL_phase3_coadapt.png"),
    ]
    ...
    for col, ylabel, filename in specs:
        ...
        ax.set_title(
            f"{ylabel} vs Steps — Fase 3 Co-adaptación: Q-Former r=128 + ViT layers 37-38 (10%)",
    ...
    for i, (name, df) in enumerate(histories.items()):
        ...
        ax.set_title(
            f"Train Loss vs Val Loss — {name} (Fase 3 Co-adaptación: r=128 + ViT layers 37-38)",
            fontsize=13, fontweight="bold",
        )
        ...
        filename_individual = f"04_train_val_loss_coadapt_{name}.png"
...
def load_phase3_datasets(processor) -> Tuple[MimicCXRDataset, MimicCXRDataset]:
    """
    Carga los datasets de train y val del 10% de MIMIC-CXR (Fase Co-adaptación).
    ...
    print("   [datasets] Cargando Train (10%) — Co-adaptación...")
    train_ds = MimicCXRDataset(csv_path=TRAIN_CSV_10PCT, **shared_kwargs)

    print("   [datasets] Cargando Val   (10%) — Co-adaptación...")
