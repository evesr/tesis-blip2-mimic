#!/usr/bin/env python3
"""
trainer_utils_phase3_10pct.py — Utilidades de entrenamiento para Fase 3.2
==========================================================================
Escalamiento Final al 10%: Optimización del Q-Former con bases LLM+ViT congeladas.
...
# Directorio raíz de salida para toda la Fase 3.2
OUTPUT_DIR      = BASE_DIR / "phase3_results-10pct"
...
def inject_qformer_lora(
    ...
    print(
        f"\n   ┌─── Parámetros del modelo (Fase 3.2: Q-Former LoRA 10%) ──┐\n"
        f"   │  Total        : {total:>15,}                          │\n"
...
class Phase3AuditCallback(TrainerCallback):
    ...
    def __init__(
        ...
        self.audit_path.write_text(
            f"# Audit Log — Fase 3.2 (10%) — {config_name}\n"
...
def train_single_config_phase3(
    ...
    print(f"\n{'='*80}")
    print(f"  FASE 3.2 (10%) — Config {config_num}/{total_configs}: {name}")
...
def plot_comparative_curves(
    ...
    specs = [
        ("Val_Loss", "Val Loss",  "01_val_loss_phase3_10pct.png"),
        ("BLEU-4",   "BLEU-4",   "02_bleu4_phase3_10pct.png"),
        ("ROUGE-L",  "ROUGE-L",  "03_rougeL_phase3_10pct.png"),
    ]
    ...
    for col, ylabel, filename in specs:
        ...
        ax.set_title(
            f"{ylabel} vs Steps — Fase 3.2: Q-Former Escalamiento Final (10%)",
    ...
    for i, (name, df) in enumerate(histories.items()):
        ...
        ax.set_title(
            f"Train Loss vs Val Loss — {name} (Fase 3.2 - 10%)",
            fontsize=13, fontweight="bold",
        )
