#!/usr/bin/env python3
"""
run_tournament_phase3.py — Mini-Torneo Fase 3: Optimización Semántica Q-Former
===============================================================================
Entrenamiento exclusivo del Q-Former con bases LLM+ViT congeladas (1% MIMIC-CXR).

Configuraciones del Torneo (4 corridas secuenciales)
----------------------------------------------------
  Config_3A1 : Base A (LLM r=16 + ViT r=16) + Q-Former LoRA (r=16, α=32)
  Config_3A2 : Base A (LLM r=16 + ViT r=16) + Q-Former LoRA (r=32, α=64)
  Config_3B1 : Base B (LLM r=32 + ViT r=8)  + Q-Former LoRA (r=16, α=32)
  Config_3B2 : Base B (LLM r=32 + ViT r=8)  + Q-Former LoRA (r=32, α=64)

Por cada corrida este script:
  1. Llama a trainer_utils_phase3.train_single_config_phase3() → entrenamiento aislado.
  2. Guarda adaptador Q-Former en phase3_results/checkpoints/best_model_{name}/qformer/.
  3. Llama a fase1_inferencia_phase3() → genera predicciones_{name}.csv en el
     Test Set del 1% cargando los 3 adaptadores (LLM + ViT + Q-Former).
...
---
    ...
    python run_tournament_phase3.py --skip-train              # solo inferencia + eval
    python run_tournament_phase3.py --skip-inference          # reutiliza predicciones existentes
    python run_tournament_phase3.py --configs Config_3A1      # procesa solo Config_3A1
    python run_tournament_phase3.py --num-samples 200         # evalúa solo N muestras del test set
...
# ── Utilidades de Fase 3 (trainer_utils_phase3.py) ───────────────────────────
from trainer_utils_phase3 import (
    OUTPUT_DIR,
    CHECKPOINTS_DIR,
    TEST_CSV_1PCT,
    IMAGES_DIR_1PCT,
...
def fase1_inferencia_phase3(
    ...
    """
    Genera reportes para el Test Set del 1% y los exporta a
    predicciones_{config_name}.csv.

    Diferencia respecto a fases anteriores
    ----------------------------------------
    Los checkpoints de Fase 3 tienen estructura triple:
    ...
    Args:
        config_name  : Nombre de la config (p. ej. "Config_3A1").
        model_dir    : Ruta a phase3_results/checkpoints/best_model_{name}/.
    ...
    logger.info(f"\n{'='*65}")
    logger.info(f"  INFERENCIA PHASE-3: {config_name}")
    ...
    # ── Test Dataset ──────────────────────────────────────────────────────────
    logger.info("  Cargando Test Set (1%)...")
    test_dataset = MimicCXRDataset(
        csv_path            = TEST_CSV_1PCT,
        images_dir          = IMAGES_DIR_1PCT,
        processor           = processor,
...
def run_tournament(
    ...
    logger.info("Cargando datasets del 1% (train + val)...")
    ...
    for i, cfg in enumerate(configs_to_run, 1):
        name = cfg["name"]
        logger.info(f"\n{'#'*70}")
        logger.info(f"  TORNEO FASE 3 — Config {i}/{total}: {name}")
        logger.info(f"{'#'*70}")
    ...
    logger.info(f"\nTorneo Fase 3 completado. Resultados en: {OUTPUT_DIR}")
    ...
    logger.info(f"  plots/01_val_loss_phase3.png  /  02_bleu4_phase3.png  /  03_rougeL_phase3.png\n")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Mini-Torneo Fase 3 — Optimización Semántica Q-Former con bases "
            "LLM+ViT congeladas (1% MIMIC-CXR)."
        ),
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "phase3_results/checkpoints/best_model_{name}/. "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "existentes en phase3_results/evaluation/."
        ),
    )
    parser.add_argument(
        "--num-samples",
...
def main() -> int:
    ...
    print("\n" + "="*80)
    print("  FASE 3 — OPTIMIZACIÓN SEMÁNTICA Q-FORMER")
    print("="*80)
    print(f"  Configuraciones: {len(configs_to_run)}")
    print(f"  Dataset: 1% MIMIC-CXR (entrenamiento ligero)")
