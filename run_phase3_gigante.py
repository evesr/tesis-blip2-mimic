#!/usr/bin/env python3
"""
run_phase3_gigante.py — Fase 3 Gigante: Expansión de Rango LoRA Masiva
=======================================================================
Entrenamiento exclusivo del Q-Former con bases LLM+ViT congeladas (10% MIMIC-CXR).

Configuración Única
--------------------
  Config_3B_Gigante : Base B (LLM r=32 + ViT r=8) + Q-Former LoRA (r=128, α=256)

Motivación: El entrenamiento previo demostró underfitting severo por falta de
capacidad dimensional en el Q-Former. Se aplica expansión de rango masiva (r=128)
sobre la mejor línea base disponible (Base B = best_model_Config_B1).

Por cada corrida este script:
  1. Llama a trainer_utils_phase3_10pct.train_single_config_phase3() → entrenamiento aislado.
  2. Guarda adaptador Q-Former en phase3_results-10pct/checkpoints/best_model_{name}/qformer/.
  3. Llama a fase1_inferencia_phase3() → genera predicciones_{name}.csv en el
     Test Set del 10% cargando los 3 adaptadores (LLM + ViT + Q-Former).
...
---
    ...
    python run_phase3_10pct.py --skip-train              # solo inferencia + eval
    python run_phase3_10pct.py --skip-inference          # reutiliza predicciones existentes
    python run_phase3_10pct.py --configs Config_Final_A  # procesa solo Config_Final_A
    python run_phase3_10pct.py --num-samples 500         # evalúa solo N muestras del test set
...
# ── Utilidades de Fase 3 Gigante (trainer_utils_phase3_gigante.py) ────────────
from trainer_utils_phase3_gigante import (
    OUTPUT_DIR,
    CHECKPOINTS_DIR,
    TEST_CSV_10PCT,
    IMAGES_DIR_10PCT,
...
# ─────────────────────────────────────────────────────────────────────────────
# 3.  DEFINICIÓN DE LAS 2 CONFIGURACIONES FINALISTAS
# ─────────────────────────────────────────────────────────────────────────────
#
# base_weights_key: "B" → best_model_Config_B1 (LLM r=32 + ViT r=8 de Fase 2)
#
# Configuración única: expansión de rango masiva para corregir underfitting
#
TOURNAMENT_CONFIGS: List[Dict] = [
    {
        "name":             "Config_3B_Gigante",
        "qformer_r":        128,
        "qformer_alpha":    256,
        "base_weights_key": "B",
    },
...
def fase1_inferencia_phase3(
    ...
    ----------------------------------------
    Los checkpoints de Fase 3.2 tienen estructura triple:
        DUAL_WEIGHTS[base_key]["llm"]   ← LLM LoRA (Fase 2)
        DUAL_WEIGHTS[base_key]["vit"]   ← ViT LoRA (Fase 2)
        model_dir/qformer/              ← Q-Former LoRA (Fase 3.2)

    Esta función carga los TRES adaptadores antes de lanzar la inferencia.
    ...
    logger.info(f"\n{'='*65}")
    logger.info(f"  INFERENCIA PHASE-3.2 (10%): {config_name}")
...
def run_tournament(
    ...
    for i, cfg in enumerate(configs_to_run, 1):
        name = cfg["name"]
        logger.info(f"\n{'#'*70}")
        logger.info(f"  TORNEO FASE 3.2 (10%) — Config {i}/{total}: {name}")
        logger.info(f"{'#'*70}")
    ...
    logger.info(f"\nFase 3 Gigante completada. Resultados en: {OUTPUT_DIR}")
    ...
    logger.info(f"  plots/01_val_loss_phase3_gigante.png  /  02_bleu4_phase3_gigante.png  /  03_rougeL_phase3_gigante.png\n")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Fase 3 Gigante — Expansión de Rango Masiva (r=128): Q-Former LoRA "
            "con bases LLM+ViT congeladas (corrección de underfitting)."
        ),
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "phase3_results-10pct/checkpoints/best_model_{name}/. "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "existentes en phase3_results-10pct/evaluation/."
        ),
    )
    parser.add_argument(
        "--num-samples",
...
def main() -> int:
    ...
    print("\n" + "="*80)
    print("  FASE 3 GIGANTE — EXPANSIÓN DE RANGO MASIVA")
    print("  Q-Former LoRA r=128, α=256 (Corrección de Underfitting)")
    print("="*80)
