#!/usr/bin/env python3
"""
run_phase3_10pct.py — Fase 3.2: Escalamiento Final al 10%
=========================================================
Entrenamiento exclusivo del Q-Former con bases LLM+ViT congeladas (10% MIMIC-CXR).

Configuraciones Finalistas (2 corridas secuenciales)
----------------------------------------------------
  Config_Final_A : Base A (LLM r=16 + ViT r=16) + Q-Former LoRA (r=32, α=64)
  Config_Final_B : Base B (LLM r=32 + ViT r=8)  + Q-Former LoRA (r=32, α=64)

Ambas configs ganaron con r=32 en la fase exploratoria del 1%.
...
# ── Utilidades de Fase 3.2 (trainer_utils_phase3_10pct.py) ───────────────────
from trainer_utils_phase3_10pct import (
    OUTPUT_DIR,
    CHECKPOINTS_DIR,
    TEST_CSV_10PCT,
    IMAGES_DIR_10PCT,
...
# ─────────────────────────────────────────────────────────────────────────────
#
# base_weights_key: "A" → best_model_Config_A2 (LLM r=16 + ViT r=16 de Fase 2)
#                   "B" → best_model_Config_B1 (LLM r=32 + ViT r=8  de Fase 2)
#
# Ambas configuraciones usan r=32, alpha=64 en el Q-Former (ganadoras del 1%)
#
...
def run_tournament(
    ...
    logger.info(f"\nTorneo Fase 3.2 (10%) completado. Resultados en: {OUTPUT_DIR}")
    ...
    logger.info(f"  plots/01_val_loss_phase3_10pct.png  /  02_bleu4_phase3_10pct.png  /  03_rougeL_phase3_10pct.png\n")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Fase 3.2 — Escalamiento Final al 10%: Q-Former LoRA con bases "
            "LLM+ViT congeladas."
        ),
...
def main() -> int:
    ...
    print("\n" + "="*80)
    print("  FASE 3.2 — ESCALAMIENTO FINAL AL 10%")
    print("  Q-Former LoRA r=32, α=64 (Configuraciones Ganadoras)")
    print("="*80)
    print(f"  Configuraciones: {len(configs_to_run)}")
    print(f"  Dataset: 10% MIMIC-CXR (escalamiento final)")
